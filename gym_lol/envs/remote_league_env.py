...
class LeagueEnvRemote(gym.Env):
    """
    Gym client to interface with gym_http_server
    """
    def __init__(self, remote_base='http://127.0.0.1:5000'): #add to config
    ...
    def env_create(self, env_id, default=True):
        route = '/v1/envs/'
        data = {'env_id': env_id, 'default': default}
    ...
    def reset(self, instance_id='default'):
        route = '/v1/envs/{}/reset/'.format(instance_id)
        resp = self._post_request(route, None)
        observation = resp['observation']
        return observation

    def step(self, action, instance_id='default', render=False):
    ...
    def env_action_space_info(self, instance_id='default'):
        route = '/v1/envs/{}/action_space/'.format(instance_id)
        resp = self._get_request(route)
        info = resp['info']
        return info

    def env_action_space_sample(self, instance_id='default'):
        route = '/v1/envs/{}/action_space/sample'.format(instance_id)
        resp = self._get_request(route)
        action = resp['action']
        return action

    def env_action_space_contains(self, x, instance_id='default'):
        route = '/v1/envs/{}/action_space/contains/{}'.format(instance_id, x)
        resp = self._get_request(route)
        member = resp['member']
        return member

    def env_observation_space_info(self, instance_id='default'):
        route = '/v1/envs/{}/observation_space/'.format(instance_id)
        resp = self._get_request(route)
        info = resp['info']
        return info

    def env_observation_space_contains(self, params, instance_id='default'):
        route = '/v1/envs/{}/observation_space/contains'.format(instance_id)
        resp = self._post_request(route, params)
        member = resp['member']
        return member

    def env_monitor_start(self, directory, instance_id='default',
    ...
    def env_monitor_close(self, instance_id='default'):
        route = '/v1/envs/{}/monitor/close/'.format(instance_id)
        self._post_request(route, None)

    def env_close(self, instance_id='default'):
...
#     client = LeagueEnvRemote(remote_base)
